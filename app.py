...
def check(email):
    url = "https://c0neox2p0b.execute-api.us-east-2.amazonaws.com/Plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://c0neox2p0b.execute-api.us-east-2.amazonaws.com/Plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://c0neox2p0b.execute-api.us-east-2.amazonaws.com/Plasma/getbloodgroupsdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://c0neox2p0b.execute-api.us-east-2.amazonaws.com/Plasma/requestonbloodgroups?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    #emailids=[]
    for i in a:
        url ="https://www.fast2sms.com/dev/bulk?authorization=weBQKBrtZzLnD2ZUEnUYJIO40zZGnjgZm3BA1SAUd0qZ56gHm0k3X45DWR9c&sender_id=FSTSMS&message=Need Plasma of your blood group&language=english&route=p&numbers="+str(i['phone'])
